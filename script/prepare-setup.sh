...
SIGNING_DIR="$PROJECT_ROOT_DIR/signing/$ENV_NAME"
echo "Creating path: $SIGNING_DIR"
mkdir -p "$SIGNING_DIR"
echo "Writing signing files"
echo "$ENV_SIGNING_PROP_FILE" | base64 -d > "$SIGNING_DIR/carl-$ENV_NAME-keystore.properties"
echo "$ENV_SIGNING_KEYSTORE_FILE" | base64 -d > "$SIGNING_DIR/carl-$ENV_NAME-key.keystore"
